function createSlidingTimeline(graphData) {
    ...
    let mouseRectangle = svg2.append('rect')
        ...
        .attr('width', 40) // Initial width, double the desired range
        .attr('height', height2)
        .attr('fill', 'transparent')
        .attr('opacity', 1)
        .attr('x', -20) // Initial x position, half of the desired range
...
// Handle mousemove event
    svg2.on('mousemove', function (event) {
        ...
        const yZeroX = xScale2.invert(0);
        // Update the position and width of the mouse rectangle
        mouseRectangle
            .attr('x', mouseX - 20) // Adjust the x position for the center
        ...
        const nodesInWindow = nodes2.filter(function (d) {
            ...
            return barX <= mouseX + 20 && barX + barWidth >= mouseX - 20;
        });
        ...
        //console.log('Start Value:', windowStartValue);
        //console.log('End Value:', windowEndValue);
        ...
        node2.attr('opacity', function (d) {
            ...
            if (barX <= mouseX + 20 && barX + barWidth >= mouseX - 20) {
...
// Handle mouseout event
    ...
    let height3 = 300 - margin3.top - margin3.bottom;
...
// Handle mousemove event
    svg3.on('mousemove', function (event) {
        ...
        svg3.append('line')
            ...
            .attr('x1', firstScaledNodeX)
            .attr('y1', 0)
            .attr('x2', firstScaledNodeX)
            .attr('y2', height3)
            .attr('stroke', 'red')
            .attr('stroke-width', 2)
            .attr('opacity', 0.5);
        ...
        svg3.append('line')
            .attr('class', 'vertical-line')
            .attr('x1', firstScaledNodeX + (lastScaledNodeX - firstScaledNodeX) * scaleFactor)
            .attr('y1', 0)
            .attr('x2', firstScaledNodeX + (lastScaledNodeX - firstScaledNodeX) * scaleFactor)
            .attr('y2', height3)
            .attr('stroke', 'blue')
            .attr('stroke-width', 2)
            .attr('opacity', 0.5);
        ...
        node3.attr('x', d => {
            ...
            if (barX <= mouseX + 100 && barX + barWidth >= mouseX - 100) {
                ...
                return scaledX + (xScale3(d.start_time) - scaledX) * scaleFactor;
            } else {
                ...
                if (barX < mouseX - 100) {
                    ...
                    // Use the offset to fill the gap
                    //return (firstScaledNodeX + (lastScaledNodeX - firstScaledNodeX) * scaleFactor);
                    const firstBeforeX = xScale3(d3.max(nodes3.filter(d => xScale3(d.start_time) < barX), d => d.end_time));
                    return barX + ((firstScaledNodeX + (lastScaledNodeX - firstScaledNodeX) * scaleFactor) - lastScaledNodeX)
                }
            }
        }).attr('width', d => {
