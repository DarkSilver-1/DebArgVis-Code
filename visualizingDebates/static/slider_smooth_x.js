...
//--------------------------------------------------------------------
    ...
    d3.forceSimulation(nodes).force('link', d3.forceLink(links).id(d => d.id).distance(30))
...
function textHoverAction(links, text, textArray, link, svg3, newText) {
    ...
    let outside_links_start_values = link.filter(l => l.source.text === text).filter(l => nodesInWindow && nodesInWindow.includes(l.source) && !nodesInWindow.includes(l.target)).data().map(l => l.target.start_time)
    let outside_nodes = svg3.selectAll('.node-group').filter(n => outside_links_start_values.includes(n.start_time));
    appendNodeText(outside_nodes)
...
function addTextBox(width3, svg3, nodes, links, link) {
    ...
    textArray.filter(x => x !== "").forEach(function (text, index) {
        let prevCutIndex = 0
        let finished = false
        let node = nodes.find(node => node.text === text)
        ...
        if (numberOfCharsInLine + text.length > maxNumOfLetters) {
            ...
            newText.attr('y', yValue + "em").attr('x', previousX).attr('id', `hovered-text-${index}`).attr('fill', "white").text(text)
            numberOfCharsInLine += text.length
            previousX += newText.node().getComputedTextLength() + small_margin

        }
        newText.on("mouseover", function () {
            textHoverAction(links, text, textArray, link, svg3, newText);
        }).on('mouseout', function () {
            textArray.forEach((t, i) => {
